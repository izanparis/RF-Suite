...
import { Upload, FileText, RefreshCw, Zap, Activity, Check, ArrowRight, Download } from 'lucide-react';
...
export function MeasurementTool({ onBackToDashboard }: MeasurementToolProps) {
  ...
  const [device, setDevice] = useState('HP-8752A');

  const architecture = device.includes('HP') ? 'HP8752A' : 'NanoVNA';
  ...
  const handleReset = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:8080/api/vna/hp/reset?device=${encodeURIComponent(device)}`, {
        method: 'POST'
      });
  ...
  const handleImportToVNA = async () => {
    if (architecture !== 'HP8752A') return;
  ...
  const handleAction = async (id: string) => {
    ...
    if (id === 'measure') {
      ...
      const maxPts = architecture === 'HP8752A' ? 1601 : 1024;
          ...
          <Card className="border-border/70 bg-[var(--rf-panel-soft)]">
            ...
            <CardContent className="space-y-2 px-4 pb-4 text-sm">
              {[
                ...
                ['Salida', 'Guarda S1P/S2P en servidor y descarga local al finalizar.'],
              ].map(([title, body]) => (
